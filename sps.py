...
def print_bottom_banner(openPorts):
    ...
    if openPorts == 0 or openPorts == 1:
        ...
        print("Scanned target has {} ports open".format(openPorts))
...
def tcp_scan(ip, port_range):
    ...
    try:
        ...
        print("ERROR: Host is not responding :(")
        print("Exiting...")
        sys.exit()

def tcp_scan_ranged(ip, lowPort, highPort):
    openPorts = 0
    try:
        print("[!] PORT     STATE     SERVICE")
...
def udp_scan_ranged(ip, lowPort, highPort):
    openPorts = 0
    try:
        print("[!] PORT     STATE     SERVICE")
        for port in range(lowPort, highPort):
...
def scan_ports(ip, portAll, udp, specificPortRange):
    ...
    try:
        ...
        #Specific Port Range TCP
            ...
            if ',' in str(specificPortRange):
                port_range = str(specificPortRange).split(',')
                port_rangeInt = [eval(x) for x in port_range]
                tcp_scan(ip, port_rangeInt)
        ...
        #Specific Port Range UDP
            ...
            print("Scanning UDP Ports from " + str(specificPortRange))

            if ',' in str(specificPortRange):
                port_range = str(specificPortRange).split(',')
                port_rangeInt = [eval(x) for x in port_range]
                udp_scan(ip, port_rangeInt)
...
def main():
    ...
    print("GitHub: https://github.com/mbrenton/Simple-Port-Scanner")
    print("-" * 55)

    #Call scan ports function
